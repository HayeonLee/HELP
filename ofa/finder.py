...
# Modified by Hayeon Lee and Sewoong Lee (https://github.com/HayeonLee/HELP) for
# help hardware-adaptive efficient latency prediction for nas via meta-learning (NeurIPS 2021)

import os
import torch
from utils import arch_encoding_ofa as arch_enc
...
class ArchManager:
    ...
    def random_resample_resolution(self, sample):
        sample['r'] = random.choice(self.resolutions)
...
class EvolutionFinder:
    ...
    def __init__(self, constraint_type, 
        ...
        self.hw_embed = hardware_embedding
        self.adapted_state_dict = adapted_state_dict
    ...
    def random_sample(self):
        ...
        while True:
            ...
            efficiency = self.efficiency_predictor(
                arch_enc(sample).view(1,-1).cuda(), self.hw_embed.view(1,-1), self.adapted_state_dict)
    ...
    def mutate_sample(self, sample):
        ...
        while True:
            ...
            efficiency = self.efficiency_predictor(arch_enc(new_sample).view(1,-1).cuda(), self.hw_embed.view(1,-1), self.adapted_state_dict)
            if efficiency <= constraint:
    ...
    def run_evolution_search(self, verbose=False):
        ...
        for iter in tqdm(range(max_time_budget), desc='Searching (%s) ' % (self.constraint_type)):
        ...
        return best_valids, best_info, sorted(population, key=lambda x: x[0])[::-1][:10]